...
-- ============================================
-- Phiên bản đầy đủ bao gồm tin nhắn, thông báo, và money request
...
CREATE TABLE transactions (
  ...
  related_message_id BIGINT -- Will add FK after messages table is created
);
...
CREATE TABLE friendships (
  ...
  is_read BOOLEAN DEFAULT FALSE, -- Trạng thái đã đọc lời mời kết bạn
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW(),
...
CREATE TABLE split_transaction_participants (
  ...
  is_read BOOLEAN DEFAULT FALSE, -- Trạng thái đã đọc thông báo chia tiền
  created_at TIMESTAMPTZ DEFAULT NOW()
);
...
-- Thêm foreign key cho related_message_id sau khi messages table đã tạo
ALTER TABLE transactions 
ADD CONSTRAINT fk_transactions_related_message 
...
CREATE INDEX idx_transactions_related_message ON transactions(related_message_id);
...
CREATE INDEX idx_friendships_read ON friendships(friend_id, is_read, status);
...
CREATE INDEX idx_split_participants_read ON split_transaction_participants(user_id, is_read);
...
COMMENT ON COLUMN friendships.is_read IS 'Trạng thái đã đọc lời mời kết bạn';
COMMENT ON COLUMN split_transaction_participants.is_read IS 'Trạng thái đã đọc thông báo chia tiền';

-- ============================================
-- HOÀN THÀNH
-- ============================================

SELECT 'Complete database with messaging and money request created successfully!' as status;

/*
DATABASE SCHEMA HOÀN CHỈNH VỚI TIN NHẮN VÀ MONEY REQUEST:
...
✅ Cột related_message_id trong transactions để link với money request
✅ Cột is_read trong friendships và split_transaction_participants
...
✅ Realtime enabled cho tin nhắn và transactions
...
- Tin nhắn realtime
- Money request (đòi tiền) với tracking chính xác
...
4. Realtime messaging và transaction tracking đã được bật
5. Money request sẽ không bị nhầm lẫn nhờ related_message_id
*/

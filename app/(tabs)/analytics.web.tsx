...
// ================== CHỌN VICTORY ĐÚNG NỀN TẢNG ==================
// ✅ Quan trọng: KHÔNG dùng Platform.select với require() trong object literal
// vì Metro có thể “đụng” require('victory') trên mobile -> component undefined.
let V: any;
...
if (Platform.OS === 'web') {
  ...
  // ✅ merge để xử lý mọi kiểu export (CJS/ESM)
  V = { ...m, ...(m?.default ?? {}) };
}

const { VictoryBar, VictoryChart, VictoryPie, VictoryAxis } = V;
...
export default function AnalyticsScreen() {
  ...
  // ✅ Nếu vẫn lỗi, bật dòng này để xem component nào undefined:
  // console.log('Victory check:', { VictoryPie, VictoryChart, VictoryAxis, VictoryBar });

  // ✅ Guard cực an toàn: nếu component bị undefined (do package lỗi) thì show fallback
  if (!VictoryPie || !VictoryChart || !VictoryAxis || !VictoryBar) {
    return (
      <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', padding: 16, backgroundColor: bg }}>
        <Text style={{ color: text, fontWeight: '900', fontSize: 16 }}>Chart chưa sẵn sàng</Text>
        <Text style={{ color: text + 'AA', marginTop: 8, textAlign: 'center' }}>
          Thiếu/ lệch version victory hoặc victory-native. Hãy chạy:
          {'\n'}npx expo install react-native-svg
          {'\n'}npm i victory victory-native
          {'\n'}npx expo start -c
        </Text>
      </View>
    );
      ...
      <View style={{ padding: 14, borderRadius: 18, backgroundColor: card, borderWidth: 1, borderColor: text + '12' }}>
        ...
        {topCategories.length === 0 ? (
          ...
          <VictoryChart height={260} padding={{ top: 20, bottom: 50, left: 60, right: 20 }} domainPadding={{ x: 18 }}>
            ...
            />
            <VictoryBar
              data={topCategories.map((x) => ({ x: x.category, y: x.total }))}
